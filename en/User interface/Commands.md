...
To register a new command for your plugin, call the [[Plugin_2#addCommand|addCommand()]] method inside the `onload()` method:
...
If your command is only able to run under certain conditions, then consider using [[Command#checkCallback|checkCallback()]] instead.
...
If your command needs access to the editor, you can also use the [[Command#editorCallback|editorCallback()]], which provides the active editor and its view as arguments.
...
If the editor callback can only run given under certain conditions, consider using the [[Command#editorCheckCallback|editorCheckCallback()]] instead. For more information, refer to [[#Conditional commands]].
...
In this example, the user can run the command by pressing and holding `Ctrl` (or `Cmd` on macOS) and Shift together, and then pressing the letter `a` on their keyboard.
