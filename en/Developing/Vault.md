...
There are two methods for reading the content of a file: [[obsidian.vault.read|read()]] and [[obsidian.vault.cachedread|cachedRead()]].
...
There are two methods to delete a file, [[obsidian.vault.delete|delete()]], and [[obsidian.vault.trash|trash()]]. Which one you should use depends on if you want to allow the user to change their mind.
...
Some operations return or accept a [[obsidian.tabstractfile|TAbstractFile]] object, which can be either a file or a folder. Always check the concrete type of a `TAbstractFile` before you use it.
